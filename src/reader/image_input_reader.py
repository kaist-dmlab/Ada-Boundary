...
class ImageReader(object):
    def __init__(self, dataset_name, data_path, num_train_files, num_train_images, num_val_images, height, width, depth, num_classes, crop_height=0, crop_width=0):
    ...
    def data_read(self, batch_size, train=True, distort=False, normalize=True, shuffle=False):
        ...
        if train:
            return generate_image_and_label_batch(t_id, t_image, t_label, batch_size, self.num_train_images, 0.4, 16, shuffle=shuffle)
        else:
            return generate_image_and_label_batch(t_id, t_image, t_label, batch_size, self.num_val_images, 0.4, 16, shuffle=shuffle)
