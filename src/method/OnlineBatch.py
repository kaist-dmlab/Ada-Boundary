...
from reader import batch_patcher_online as online_patcher
from network.WideResNet.WideResNet import *
from network.DenseNet.DenseNet import *
os.environ['TF_CPP_MIN_LOG_LEVEL']='3'
tf.logging.set_verbosity(tf.logging.ERROR)

def OnlineBatch(gpu_id, input_reader, model_type, training_epochs, batch_size, lr_boundaries, lr_values, optimizer_type, warm_up_period, s_e=20.0, pretrain=0, log_dir="log"):
    ...
    train_batch_patcher = online_patcher.BatchPatcher(num_train_images, batch_size, num_label, s_e=s_e, update_method=update_method)
    validation_batch_patcher = online_patcher.BatchPatcher(num_val_images, batch_size, num_label, update_method=update_method)
                ...
                for epoch in range(pretrain, training_epochs):
                    ...
                    if epoch < warm_up_period:
                        ...
                        for i in range(train_batch_patcher.num_iters_per_epoch):
                            ...
                            xentropy = sess.run(test_xentropy_op, feed_dict={model.test_image_placeholder: images, model.test_label_placeholder: labels})
                        ...
                        if len(loss_map) != input_reader.num_train_images:
                            print("error: size of loss map is not equal to the size of data set.")
                        train_batch_patcher.update_importance_table_based_on_online_bs(loss_map)
                        ###########################################################################################################################################################
